...
def handle_connection(s):
    if RP == 1:
        ...
        create_connection_with_father_node(port)
        package = pack("REQ", "new_connection", self_addr[0], port, host, port)
        s.sendall(package)
...
def create_connection_with_father_node(con_port):
    ...
    while CON_STATUS != 1:
        conn, addr = s.accpet()
        CON_STATUS = 1
    thread = threading.Thread(target=handle_msg_for_normal_node, args=(conn, addr,))
    thread.start()
    return;
